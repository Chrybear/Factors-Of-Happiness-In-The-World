...
# Add a mean for social welfare
avg_happy$Social.Mean <- (avg_happy$Social.2015 + avg_happy$Social.2016 + avg_happy$Social.2017 + avg_happy$Social.2018 + avg_happy$Social.2019)/5

# Get top 10 happiest overall
top_ten_overall <- head(avg_happy %>% arrange(desc(Score.Mean)), 10)


ggplot(data = top_ten_overall, aes(x = Social.Mean, y = Health.Mean)) +
...
ggplot(data = bottom_ten_overall, aes(x = Social.Mean, y = Health.Mean)) +
  labs(title = "Correlation of Welfare and Health Among bottom 10 Happiest", x = "Welfare", y ="Health") +
  theme(plot.title = element_text(hjust = 0.5)) + geom_line()

# Among entire data
ggplot(data = avg_happy, aes(x = Social.Mean, y = Health.Mean)) +
